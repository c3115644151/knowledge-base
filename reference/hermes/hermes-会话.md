...
- 消息历史（包括完整消息记录，gateway canonical store）
- 跨压缩/恢复的会话谱系
- `session_search` 使用的全文搜索索引

> ⚠️ **JSONL 已废弃**：Sessions 创建的 JSONL 文件 (`~/.hermes/sessions/*.jsonl`) 已不再被 Hermes 读写。state.db 是消息的唯一 canonical store。在确认对应会话存在于 state.db 后，可安全删除旧 JSONL 文件。
